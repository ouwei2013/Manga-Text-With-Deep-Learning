...
from edge_detector import connected_components as cc, defaults, arg
...
def form_canny_mask(rgb, img, mask=None):
    bounding_boxes = []
    ...
    temp_mask_1 = np.zeros(img.shape, np.uint8)
    for c in contours:
        ...
        # cv2.rectangle(rgb, (x, y), (x + w, y + h), (255, 0, 0), 4)
        # plt.imshow(rgb)
        # plt.show()
    ...
    return temp_mask, bounding_boxes
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--mask', dest='mask', default=None,
                        help='Output (binary) mask for non-graphical regions.')
