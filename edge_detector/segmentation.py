...
from edge_detector import clean_page as clean, connected_components as cc, defaults, furigana, ocr, arg, \
    run_length_smoothing as rls


# from edge_detector.mser import mser, color_hist_single, compute_contrast


# from skimage.feature import hog
# from sklearn import preprocessing
# from deep_learning_model import load_model,predict
...
def segment_image(rgb, img, max_scale=defaults.CC_SCALE_MAX, min_scale=defaults.CC_SCALE_MIN):
    (h, w) = img.shape[:2]
    ...
    # max_size=1300
    # print('max_size%f'%max_size)
    min_size = average_size * min_scale
    # print('min_size%f'%min_size)

    # primary mask is connected components filtered by size
    # mser_mask=0
    # mser_mask,bad_mask = mser(img,0.72)
    # print('mser finished')

    # mser_components = cc.get_connected_components(rls.RLSO(mser_mask, 50, 50))
    # mser_components = cc.get_connected_components(mser_mask)
    # mser_mask = np.zeros(mser_mask.shape,np.uint8)#,'B')
    # for component in mser_components:
    #   xs=component[1]
    #   ys=component[0]
    #   mser_mask[ys.start:ys.stop,xs.start:xs.stop]=1

    # (text_x_start,text_y_start,text_x_stop,text_y_stop)=possible_text_area
    # reference_hist =color_hist_single(gaussian_filtered[text_y_start:text_y_stop,text_x_start:text_x_stop])
    mask = cc.form_mask(gaussian_binary, max_size, min_size)  #### remove region whose area are outside
    # ax1=plt.subplot(1,3,1)
    # ax1.imshow(mask)

    # secondary mask is formed from canny edges
    canny_mask, bounding_boxes = clean.form_canny_mask(rgb, gaussian_filtered,
                                                       mask=mask)  #### detect contour based on edges and size
    # ax2=plt.subplot(1,3,2)
    # ax2.imshow(canny_mask)


    # final mask is size filtered connected components on canny mask
    final_mask = cc.form_mask(canny_mask, max_size, min_size)  ######
    # final_mask= mask_filter(final_mask,mser_mask)
    ...
    text_only = cleaned2segmented(cleaned, img, average_size)
    # text_only = rls.RLSO(text_only, 50, 50)
    ...
    # svm_clf = load('clf.pkl')


    # model = load_model()

    (text_like_areas, nontext_like_areas) = filter_text_like_areas(img,
    ...
    # text_only = filter_text_like_areas(img,gaussian_binary, text_only,average_size,svm_clf)
...
def cleaned2segmented(cleaned, img, average_size):
    img_size = (cleaned.shape[0] * cleaned.shape[1]) ** 0.5
    min_text_area_size = img_size / 100.0
    ...
    for component in components:
        ...
        # y_start=ys.start
        # y_stop =ys.stop
        # com_hist = color_hist_single(img[y_start:y_stop,x_start:x_stop])
        ...
        # if (cc.area_bb(component)**0.5)>min_text_area_size and (component_filter(component,mser_mask) or hist_sim(com_hist,reference_his)>0.99):
        # print(compute_contrast(img[y_start:y_stop,x_start:x_stop]))

        if (cc.area_bb(component) ** 0.5) > min_text_area_size:
...
def filter_text_like_areas(img, segmentation, average_size):
    ...
    max_text_like_area_size = img_size / 3.0
    min_text_like_area_size = img_size / 50.0
    ...
    text_like_areas = [area for area in text_like_areas if (cc.area_bb(area) ** 0.5) < max_text_like_area_size
                       and (cc.area_bb(area) ** 0.5) > min_text_like_area_size]
    # cnn_results=filter_text_like_areas_cnn(gaussian_binary,text_like_areas,model)
    # print(cnn_results)
    # text_like_areas=[area for (idx, area) in enumerate(text_like_areas) if cnn_results[idx]>0.1]
...
def main():
    ...
    parser.add_argument('-v', '--verbose', help='Verbose operation. Print status messages during processing',
                        action="store_true")
    parser.add_argument('--display', help='Display output using OPENCV api and block program exit.',
                        action="store_true")
    ...
    parser.add_argument('--binary_threshold', help='Binarization threshold value from 0 to 255.', type=float,
                        default=defaults.BINARY_THRESHOLD)
    parser.add_argument('--furigana', help='Attempt to suppress furigana characters to improve OCR.',
                        action="store_true")
    parser.add_argument('--segment_threshold', help='Threshold for nonzero pixels to separete vert/horiz text lines.',
                        type=int, default=defaults.SEGMENTATION_THRESHOLD)
    parser.add_argument('--additional_filtering',
                        help='Attempt to filter false text positives by histogram processing.', action="store_true")
